...
const Tickets = ({ searchTerm }) => {
  ...
  const { widgets } = newDatas.Ticket;
  const [onHover, setOnHover] = useState(null);
  const [title, setTitle] = useState("");
  const [color, setColor] = useState("Black");
  const [fields, setFields] = useState([{ name: "", value: "" }]);

  const addNewTicketWidgets = (event) => {
    ...
    dispatchData(addNewTicket);
    setTitle("");
    setFields([{ name: "", value: "" }]);
    setNewWidgetTemplate(false);
  };

  const deleteTicketWidget = (event, id) => {
    event.preventDefault();
    const deleteTicketWidget = {
      type: "DEL_TICKET",
      payload: id,
    };
    dispatchData(deleteTicketWidget);
  };
        ...
        return (
          ...
          >
            <span
              ...
              onClick={(e) => deleteTicketWidget(e, item.id)}
            >
              <RxCross2 />
            </span>
            ...
            <ResponsiveContainer width="100%" height={240}>
              <ComposedChart data={item?.data}>
                ...
                <Tooltip />
                <Legend />
                <Bar dataKey="value" barSize={40} fill={item?.color} />
